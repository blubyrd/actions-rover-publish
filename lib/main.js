...
const downloadArtifact = (name) => __awaiter(void 0, void 0, void 0, function* () {
    const client = artifact.create();
    const options = { createArtifactFolder: false };
    yield client.downloadArtifact(name, __dirname, options);
...
const getInput = () => {
    ...
    if (!path && !artifact)
        throw new Error('path or artifact inputs are required');
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const schema = artifact ? yield downloadArtifact(artifact) : path;
            const args = ['--schema', schema];
            if (federated)
                args.push('--name', subgraph);
            if (routingURL)
                args.push('--routing-url', routingURL);
            if (convert)
                args.push('--convert');
            yield rover([
